...
async function bootstrap() {
  ...
  const explicitOrigins = (process.env.FRONTEND_URLS || process.env.FRONTEND_URL || '')
    ...
    .map((x) => x.trim())
    .filter(Boolean);
  const allowlist = new Set<string>(['http://localhost:3000', 'https://crm-suites-o7.vercel.app', 'https://www.kabinfinancial.com.mx', 'https://kabinfinancial.com.mx', ...explicitOrigins]);
  app.enableCors({
